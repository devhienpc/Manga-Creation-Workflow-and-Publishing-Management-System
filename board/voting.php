...
.stat-mini.approved .num { color: #10b981; }
.stat-mini.rejected .num { color: #ef4444; }
...
.page-strip::-webkit-scrollbar-thumb { background: rgba(255,255,255,.15); border-radius: 4px; }
.page-thumb {
    flex-shrink: 0;
...
.page-thumb:hover { border-color: rgba(99,102,241,.5); }
.page-thumb.active { border-color: var(--accent-primary); box-shadow: 0 0 8px rgba(99,102,241,0.4); }
.page-thumb .pnum {
    position: absolute; bottom: 0; left: 0; right: 0;
    background: rgba(0,0,0,.7); font-size: .6rem; text-align: center;
...
.series-cover img { width:100%; height:100%; object-fit:cover; border-radius: 8px; }
.genre-tag {
...
.vote-btn.selected .check-mark { display: block; }
...
.schedule-opt:hover { border-color: rgba(99,102,241,0.4); color: var(--text-primary); }
.schedule-opt.selected {
    border-color: var(--accent-primary);
    background: rgba(99,102,241,0.12);
    color: #a5b4fc;
}
.schedule-opt .icon { font-size: 1.4rem; display: block; margin-bottom: 4px; }
...
.approve-fields.visible { display: block; }
@keyframes slideDown {
...
.no-selection-state svg { opacity: 0.3; }

/* Loading spinner */
.vote-loading { display: none; }
.vote-loading.active { display: flex; align-items: center; gap: 8px; }
...
.section-heading h3 { font-size: 1rem; font-weight: 700; color: #fff; margin: 0; }
...
@media (max-width: 1024px) {
    ...
    .series-info-grid { grid-template-columns: 80px 1fr; }
}
</style>
...
<div class="tab-row">
    <button class="tab-btn active" id="tabVoting" onclick="switchTab('voting')">
        ...
        <?php if ($pendingCount > 0): ?>
            <span style="background:#f59e0b; color:#0b0b16; border-radius:10px; padding:1px 6px; font-size:.65rem; font-weight:800; margin-left:4px;"><?= $pendingCount ?></span>
        <?php endif; ?>
    </button>
    <button class="tab-btn" id="tabHistory" onclick="switchTab('history')">
        📋 Lịch sử bỏ phiếu <span style="background:rgba(255,255,255,0.1); border-radius:10px; padding:1px 6px; font-size:.65rem; font-weight:700; margin-left:4px;"><?= count($historyList) ?></span>
    </button>
</div>
                        ...
                        <a href="?id=<?= $sub['id'] ?>"
                           ...
                           id="subcard-<?= $sub['id'] ?>">
                            ...
                            <div class="sub-card-meta">
                                ...
                                <span style="background:rgba(245,158,11,0.15); color:#fcd34d; border-radius:4px; padding:1px 6px; font-size:.65rem; font-weight:700;">PENDING</span>
                            </div>
                        </a>
                    <?php endforeach; ?>
                    </div>
        ...
        <div class="voting-main">
            <?php if ($selectedSub): ?>
                ...
                <div class="card" style="padding:20px;">
                    <div class="section-heading">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                        <h3>Thông tin tác phẩm</h3>
                    </div>
                    <div class="series-info-grid">
                        <div class="series-cover">
                            <?php if ($selectedSub['cover_image'] && file_exists(dirname(__DIR__) . '/' . $selectedSub['cover_image'])): ?>
                                <img src="<?= BASE_URL . htmlspecialchars($selectedSub['cover_image']) ?>" alt="cover">
                            <?php else: ?>
                                📚
                            <?php endif; ?>
                        </div>
                        <div>
                            <h2 style="font-size:1.2rem; font-weight:800; color:#fff; margin-bottom:6px;"><?= htmlspecialchars($selectedSub['series_title']) ?></h2>
                            ...
                            <div style="display:flex; flex-wrap:wrap; gap:12px; font-size:0.8rem;">
                                <div>
                                    ...
                                    <strong style="color:#fff;"> <?= htmlspecialchars($selectedSub['mangaka_name']) ?></strong>
                                </div>
                                <div>
                                    <span style="color:var(--text-muted);">Đề xuất bởi:</span>
                                    <strong style="color:#a5b4fc;"> <?= htmlspecialchars($selectedSub['editor_name']) ?></strong>
                                </div>
                                <div>
                                    <span style="color:var(--text-muted);">Ngày đệ trình:</span>
                                    <strong style="color:#fcd34d;"> <?= date('d/m/Y H:i', strtotime($selectedSub['submitted_at'])) ?></strong>
                                </div>
                                <div>
                                    <span style="color:var(--text-muted);">Phiên bản bản thảo:</span>
                                    <span class="badge badge-gray" style="font-size:.7rem; padding:2px 7px;">v<?= $selectedSub['version'] ?></span>
                                </div>
                            </div>
                            <?php if (!empty($selectedSub['board_notes'])): ?>
                                <div style="margin-top:12px; padding:10px 14px; background:rgba(99,102,241,0.08); border:1px solid rgba(99,102,241,0.2); border-radius:8px; font-size:0.82rem; color:#c7d2fe;">
                ...
                <div class="card" style="padding:20px;">
                    <div class="section-heading">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#a5b4fc" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                        <h3>Đọc Bản Thảo</h3>
                        <a href="<?= BASE_URL . htmlspecialchars($selectedSub['file_path']) ?>"
                        ...
                        <div class="manuscript-viewer">
                            <iframe src="<?= $fileUrl ?>#toolbar=1&navpanes=0&scrollbar=1"
                                ...
                                <p style="padding:20px; color:var(--text-muted);">
                                    ...
                                    <a href="<?= $fileUrl ?>" target="_blank" style="color:var(--accent-primary);">Mở file trực tiếp</a>
                                </p>
                            </iframe>
                        </div>
                    <?php elseif (!empty($pages)): ?>
                        <!-- Image Page Viewer -->
                        <div class="manuscript-viewer" style="background: #05050c; border-radius:10px; border:1px solid var(--border); overflow:hidden;">
                            <!-- Thumbnail strip -->
                                ...
                                <?php foreach ($pages as $pg): ?>
                                    <div class="page-thumb <?= ($pg['id'] === ($pages[0]['id'] ?? 0)) ? 'active' : '' ?>"
                                        ...
                                        <?php if ($pg['original_file']): ?>
                                            <img src="<?= BASE_URL . htmlspecialchars($pg['original_file']) ?>" alt="Trang <?= $pg['page_number'] ?>" loading="lazy">
                                        <?php else: ?>
                                            <div style="height:100%; display:flex; align-items:center; justify-content:center; font-size:1.2rem; opacity:0.3;">📄</div>
                                        <?php endif; ?>
                            ...
                            <div class="page-main-display" id="pageDisplay">
                                <?php if (!empty($pages[0]['original_file'])): ?>
                                    <img id="mainPageImg"
                                         src="<?= BASE_URL . htmlspecialchars($pages[0]['original_file']) ?>"
                    ...
                    <?php else: ?>
                        <div style="padding:40px; text-align:center; background:var(--bg-input); border-radius:8px; border:1px dashed var(--border); color:var(--text-muted);">
                            ...
                            <?php if ($filePath): ?>
                                <a href="<?= $fileUrl ?>" target="_blank" class="btn btn-secondary" style="margin-top:16px; display:inline-flex;">
                                    Mở file bản thảo
                                </a>
                            <?php endif; ?>
                        </div>
                ...
                <div class="vote-form-card" id="voteFormCard">
                    ...
                    <!-- Vote Buttons -->
                    <label style="font-size:0.75rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:.06em; display:block; margin-bottom:10px;">
                    ...
                    <div class="approve-fields" id="approveFields">
                        ...
                        <div class="schedule-row">
                            <div class="schedule-opt" id="optWeekly" onclick="selectSchedule('weekly')">
                                ...
                                Hàng tuần
                                <div style="font-size:0.7rem; color:var(--text-muted); margin-top:3px;">Mỗi tuần 1 chương</div>
                            </div>
                            <div class="schedule-opt" id="optMonthly" onclick="selectSchedule('monthly')">
                                <span class="icon">🗓️</span>
                                Hàng tháng
                                <div style="font-size:0.7rem; color:var(--text-muted); margin-top:3px;">Mỗi tháng 1 chương</div>
                            </div>
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom:16px;">
                            ...
                            <input type="date" id="publishDateInput" class="form-control"
                                   min="<?= date('Y-m-d') ?>"
                    ...
                    <div style="display:flex; gap:12px; align-items:center;">
                        <button class="btn btn-primary" id="btnSubmitVote" onclick="submitVote()"
                            ...
                            <span id="btnVoteText">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-3px;"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                                Xác nhận bỏ phiếu
                            </span>
                            <span id="btnVoteLoading" style="display:none;">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 1s linear infinite; vertical-align:-3px;"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                                Đang xử lý...
                            </span>
                        </button>
                        ...
                        <div style="margin-left:auto; font-size:0.8rem; color:var(--text-muted);">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                ...
                <div class="no-selection-state">
                    ...
                    <?php if (empty($pendingSubmissions)): ?>
                        ...
                        <p style="font-size:0.85rem; max-width:400px; line-height:1.6;">
                            Hiện tại không có tác phẩm nào đang chờ Ban biên tập xét duyệt. Khi Biên tập viên đề xuất xuất bản một bản thảo, chúng sẽ xuất hiện ở đây.
                        </p>
                    <?php else: ?>
                        <h3 style="color:var(--text-secondary); font-size:1.1rem; margin:0;">Chọn một đệ trình để bắt đầu bỏ phiếu</h3>
                        <p style="font-size:0.85rem; max-width:400px; line-height:1.6;">
                            Nhấn vào một trong các đệ trình ở danh sách bên trái để xem thông tin chi tiết và bỏ phiếu quyết định xuất bản.
                        </p>
...
<div class="tab-panel" id="panelHistory">
    <div class="card" style="padding:0; overflow:hidden;">
        <div style="padding:20px 24px; border-bottom:1px solid var(--border);">
            ...
            <p style="margin:4px 0 0; font-size:0.8rem; color:var(--text-muted);">Tất cả quyết định phê duyệt / từ chối xuất bản được ghi lại dưới đây.</p>
        </div>
                ...
                <table style="width:100%; border-collapse:collapse;">
                    <thead>
                        <tr style="border-bottom:1px solid var(--border); text-align:left;">
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;">#</th>
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;">Tác phẩm</th>
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;">Họa sĩ</th>
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;">Đề xuất bởi</th>
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase; text-align:center;">Quyết định</th>
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;">Lịch XB</th>
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;">Ghi chú</th>
                            <th style="padding:13px 18px; font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;">Ngày</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php foreach ($historyList as $i => $h): ?>
                        <tr style="border-bottom:1px solid rgba(255,255,255,0.03); transition:background 0.15s;"
                            ...
                            <td style="padding:13px 18px;">
                                <div style="font-weight:700; font-size:0.88rem; color:#fff;"><?= htmlspecialchars($h['series_title']) ?></div>
                            </td>
                            ...
                            <td style="padding:13px 18px; text-align:center;">
                                <?php if ($h['status'] === 'approved'): ?>
                                    <span class="badge history-status-approve" style="font-size:.7rem; padding:3px 10px;">✅ Đã duyệt</span>
                                <?php else: ?>
                                    <span class="badge history-status-reject" style="font-size:.7rem; padding:3px 10px;">❌ Từ chối</span>
                                <?php endif; ?>
                            </td>
